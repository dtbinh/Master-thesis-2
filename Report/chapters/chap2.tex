...
Let
\begin{align}
\bm{u}_{t:t+N|t}^{*,j}&=[\bm{u}_{t|t}^{*,j},...,\bm{u}_{t+N-1|t}^{*,j}]\\
...
As the initial state depends on the final state of the previous iteration for repetitive systems, it can not be constrained to a single state $\bm{x}_S$ (as in \eqref{eq:generalProblem3}). The problem from Eq. \eqref{eq:generalProblem} therefore becomes the following problem:
...
Under the assumption of no model mismatch the proposed method is feasible, stable, and it converges to an optimal solution. In general, the method can be applied to various repetitive systems. However, the important assumption \ref{as:nonincLMPC} has to be verified individually in order to guarantee non-increasing cost.
